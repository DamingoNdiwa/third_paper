...
Using the concept of the power posterior we construct a continuous family of
posteriors between prior and posterior where the usual likelihood is raised to
a varying parameter $\gamma \in [0, 1]$. We give an initial visual idea of this
transition in \cref{fig:skew_normal_powpos} where we show density plots of the
power posteriors induced by various $\gamma$ values as 
...
\caption{Marginal posteriors for the skewness parameter show the transition
	from Uniform prior ($\gamma=0$) to various power posteriors
	($0< \gamma \leq 0.5$). Posterior based on the frontier data set for the
	skew-normal distribution.}\label{fig:skew_normal_powpos}
\end{figure}

We then derive conjugate results for the power posterior including the Normal,
Inverse-Gamma and Poisson-Gamma cases. For a dataset of size $n$ these
conjugate results give an intepretation of the product $\gamma n$ as the
effective sample size. This intepretation
