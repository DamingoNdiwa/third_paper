...
\title{Gaining insights into prior to posterior transitions through visualizations and Wasserstein distance}
%\subtitle{Interpretation of the Wasserstein Impact Measure}
...
\section{Methodology}
The central concept in this paper is power posteriors. For Bayesian inference,
the sampling distribution can be raised to values between zero and one
inclusive. The resulting posteriors are known as power posteriors. For power
posteriors, the Bayes' theorem can be restated as shown in \cref{eq:pow_pos}.
...
where $p(\theta|y)$ is the power posterior, $p(y|\theta)$ is the likelihood
function, $p(\theta)$ is the prior distribution, $z$ is the marginal
likelihood, and $\gamma$ is a value that controls the influence of the data.
...
posterior inference.  The \gls{wim} and \gls{swim} require the calculation of
